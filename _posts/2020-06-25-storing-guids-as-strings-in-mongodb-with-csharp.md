...
One thing we'll notice is that the GUID is store as [`BinData`](https://docs.mongodb.com/manual/reference/bson-types/), If we execute the following C# we'll get a document in our collection with BinData for the `_id` field.
...
Having the field as BinData makes it hard work when reading the document, however there is an method on BinData object to return a hex string of the values.
...
There's a bunch of javascript helper functions within [uuidhelpers.js](https://github.com/mongodb/mongo-csharp-driver/blob/master/uuidhelpers.js) that can be used in the mongo console to convert these BinData back and forth between a C# GUID and using them for atomic operations. 
...
However, for some compatibility between systems you might also want to store your GUID as a string representation. This can have bigger performance implications due the the size of a stringified versions is a whopping 36 bytes compared to 128 bits! But depending on your system this might be required.

## Globally replacing the serializer

One of the ways to serialize GUIDs in C# to strings in MongoDB is to replace the GUID Serializer. This can we do this by registering the `GuidSerializer` with a constructor argument of `BsonType.String`.
...
Now if we drop in to the console we'll see the document now as a string for it's GUID.
```javascript
...
This approach is a bit brutal as anything within your system that wants to serializes to BSON will be using a string representation for GUIDs.

To note this approach also works fine with `Nullable<Guid>` too.
...
The previous example we set a plain GUID to serialize as string, However if we just swap the serializer of a Nullable<Guid> (Guid?) it will throw an exception
...
This is because we're trying to tell the GuidSerializer to serialize a nullable guid in which it has no idea what to do. What we need to do here is wrap it in a `NullableSerializer` which will take care of the null and unwrap the value.
...
We can create a simple convention of our own that checks each property and if it's a `Guid` or a `Nullable<Guid>` type and set the correct serializer.
...
Now if we insert our document again in to the collection we'll get our GUIDs serialzed as strings.
...
In my person experience if you need to store your IDs as GUIDs I'd go for setting the `BsonDefaults.GuidRepresentation` to `GuidRepresentation.Standard`, that way your GUIDs across the system will the stored in a standard way which is also more accessible from other systems and more performance than using string.

However if you need to use strings for cross system compatibility then I'd go down the route of creating a conversions that suit you needs. That way if you take on a 3rd party library in your software that uses MongoDB driver also then you don't get weird serialization issues.
