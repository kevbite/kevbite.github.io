...
After we have ran our tests we will also need to tear down our application instance. 
...
We can then use the `FabricApplicationDeployer` and `MsBuildPacker` within a _NUnit_ `SetUpFixture` so that it will run once before all of our tests.
...
As you can see I've wrapped away how we find the paths to the application within a `PathFinder` which returns a `ProjectPaths`.
...
This is a little simple but from this we can start testing our application end-to-end, either it be a API or an MVC website.